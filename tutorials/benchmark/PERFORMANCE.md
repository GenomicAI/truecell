...
> is Seurat's per-cell test now, so DESeq2 has a row for the first time
> ([section 5](#5-named-operations)).
...
wins seven of the eight: `roc` 8.9x, `bimod` 6.4x, `MAST` 3.8x, `t` 3.4x,
`negbinom` 2.7x, `DESeq2` 2.5x, `LR` 1.7x. It loses `wilcox` by 3.7x, which is
presto's C++ Wilcoxon against `scipy.stats.mannwhitneyu`. `negbinom` has been
truecell's own GLM since 2.0.0, where it was statsmodels', and stays 2.7x ahead.

**DESeq2 has a row now.** In 1.0.0 the two tools meant different things by the
name and truecell's had no row here; since 2.0.0 `test_use="deseq2"` is Seurat's
per-cell `DESeq2DETest`. It is 2.5x faster (4.00s against 10.01s) on half
Seurat's memory, and it runs in one process, as DESeq2 does in R. Left to its
default, pydeseq2 starts one worker process per core. The first measurement for
this report caught that: with 18 workers the call took 2.50s, but the process
tree went from under 1 GB to 5.2 GB and stayed there after the call returned,
for the same output. 2.0.0 gives pydeseq2 one worker.
...
* **`lazy` is not a 21x loss.** The Python script runs eight DE tests on an
  ...
  sooner because there is less it can do. Its Python peak fell from 11.5 GB to
  5.6 GB when DESeq2 moved to one process (section 5).
* **`de`** runs all nine tests on both sides, and Seurat's `MAST`, `DESeq2` and
  `negbinom` are most of its 304.4s. Its Python peak fell from 7.2 GB to 3.1 GB
  when DESeq2 moved to one process.
...
**Truecell wins seven of the eight shared DE tests** by 1.7–8.9x, and Moran's I
...
**3. ~~Limit pydeseq2's worker processes.~~ Done in 2.0.0 — see section 5.**
One worker per core bought 1.5s on the DE bench and cost 4 GB that stayed
resident after the call, more on a machine with more cores.

**4. Consider a fast path for Wilcoxon.** `scipy.stats.mannwhitneyu` is 3.7x
